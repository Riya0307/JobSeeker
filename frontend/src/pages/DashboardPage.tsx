...
export default function DashboardPage() {
  ...
  useEffect(() => { let active = true; setInterviews((state) => ({ ...state, loading: true, error: "" })); interviewsApi.listInterviews({ upcoming: true, pageSize: 3 }).then((data) => { if (active) setInterviews({ data, loading: false, error: "" }); }).catch((reason) => { if (active) setInterviews({ data: null, loading: false, error: getApiError(reason) }); }); return () => { active = false; }; }, [interviewRetry]);
