...
interface JobCardProps {
  ...
  detailState?: { fromMatching: boolean };
}

export default function JobCard({ job, busy = false, onToggleSave, children, headerExtra, detailState }: JobCardProps) {
  return (
    <article className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 transition hover:border-cyan-400/25 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">{headerExtra}{job.is_saved && <span className="rounded-full bg-cyan-400/10 px-2.5 py-1 text-xs font-medium text-cyan-300">Saved</span>}</div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-2 border-t border-white/10 pt-5 sm:flex-row">
        <Link to={`/jobs/${job.id}`} state={detailState} className="rounded-lg bg-cyan-400 px-4 py-2.5 text-center text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">View job</Link>
