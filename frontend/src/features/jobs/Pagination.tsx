...
export default function Pagination({ page, count, pageSize = 10, onChange, ariaLabel = "Job results pages" }: PaginationProps) {
  const pages = Math.max(1, Math.ceil(count / pageSize));
  if (pages <= 1) return null;
  return (
    <nav aria-label={ariaLabel} className="mt-8 flex items-center justify-between gap-4 border-t border-white/10 pt-6">
